...
export class OfficialInochi2dSdk {
    ...
    static async create(wasmUrl, signal) {
        ...
        return new OfficialInochi2dSdk(assertOfficialExports(result.instance.exports));
    }
    loadPuppet(bytes) {
        this.#assertActive();
        if (bytes.byteLength === 0 || bytes.byteLength > 0xffff_ffff) {
            throw new Error("model byte length is invalid for the official Inochi2D SDK");
        }
        const modelPointer = this.#allocate(bytes.byteLength);
        let puppetPointer = 0;
        try {
            this.#bytes(modelPointer, bytes.byteLength).set(bytes);
            puppetPointer = this.#exports.in_puppet_load_from_memory(modelPointer, bytes.byteLength, 0);
        }
        finally {
        ...
        try {
            return {
                ...
                physics: {
                    enabled: this.#exports.in_puppet_get_physics_enabled(puppetPointer) !== 0,
                    driverCount: this.#exports.in_puppet_get_driver_count(puppetPointer),
                    resolvedDriverCount: this.#exports.in_puppet_get_resolved_driver_count(puppetPointer),
                    springDriverCount: this.#exports.in_puppet_get_spring_driver_count(puppetPointer),
                    xyDriverCount: this.#exports.in_puppet_get_xy_driver_count(puppetPointer),
                    nonzeroScaleCount: this.#exports.in_puppet_get_nonzero_physics_scale_count(puppetPointer),
                },
            };
        }
        catch (error) {
    ...
    setParameter(parameter, value) {
        this.#assertActive();
        const pointer = this.#scratch(parameter.dimensions * Float32Array.BYTES_PER_ELEMENT);
        const view = new DataView(this.#exports.memory.buffer);
        if (typeof value === "number") {
            if (parameter.dimensions === 1) {
                view.setFloat32(pointer, clampParameterValue(parameter, 0, value), true);
            }
            else {
                const current = this.#readParameterValue(parameter);
                view.setFloat32(pointer, clampParameterValue(parameter, 0, value), true);
                view.setFloat32(pointer + 4, clampParameterValue(parameter, 1, current[1] ?? 0), true);
            }
        }
        else {
            view.setFloat32(pointer, clampParameterValue(parameter, 0, value[0]), true);
            if (parameter.dimensions === 2) {
                view.setFloat32(pointer + 4, clampParameterValue(parameter, 1, value[1]), true);
            }
        }
    ...
    readParameterValue(parameter) {
        this.#assertActive();
        return this.#readParameterValue(parameter);
    }
    setPhysicsEnabled(puppet, enabled) {
        this.#assertActive();
        this.#exports.in_puppet_set_physics_enabled(puppet.pointer, enabled ? 1 : 0);
     ...
     */
    parameterHasDrawEffect(puppet, parameter, axis) {
        this.#assertActive();
        ...
        try {
            this.setParameter(parameter, parameter.dimensions === 1 ? start[0] : [start[0], start[1]]);
            const before = snapshotDrawList(this.updateAndDraw(puppet, 0));
            this.setParameter(parameter, parameter.dimensions === 1 ? end[0] : [end[0], end[1]]);
            return drawListsDiffer(before, this.updateAndDraw(puppet, 0));
        }
        finally {
            this.setParameter(parameter, parameter.dimensions === 1
                ? original[0]
                : [original[0], original[1]]);
            this.updateAndDraw(puppet, 0);
        }
    }
    updateAndDraw(puppet, deltaSeconds) {
        this.#assertActive();
        ...
        for (const [name, parameter] of puppet.parameters) {
            const invalid = this.#exports.in_parameter_get_invalid_binding_application_count(parameter.pointer);
            if (invalid > 0) {
                throw new Error(`parameter ${name} produced ${invalid} invalid runtime binding value(s)`);
            }
        }
    ...
    readTextures(puppet) {
        ...
        if (!Number.isInteger(count) || count < 0 || count > 4_096) {
            throw new Error(`invalid texture count returned by the official SDK: ${count}`);
        }
        const textures = [];
        for (let slot = 0; slot < count; slot += 1) {
            ...
            if (!Number.isInteger(width) ||
                ...
                width > 4_096 ||
                height > 4_096) {
                throw new Error(`texture ${slot} has invalid dimensions ${width}x${height}`);
            }
            if (channels !== 1 &&
                channels !== 2 &&
                channels !== 3 &&
                channels !== 4) {
                throw new Error(`texture ${slot} has unsupported channel count ${channels}`);
            }
            const byteLength = checkedProduct(width, height, channels);
            if (byteLength > MAX_DRAW_BUFFER_BYTES) {
                throw new Error(`texture ${slot} exceeds the decoded texture byte limit`);
            }
    ...
    #readParameters(puppet) {
        const countPointer = this.#scratch(4);
        const listPointer = this.#exports.in_puppet_get_parameters(puppet, countPointer);
        const count = new DataView(this.#exports.memory.buffer).getUint32(countPointer, true);
        if (count > MAX_PARAMETERS) {
            throw new Error(`model exposes too many parameters: ${count}`);
        }
        const pointers = this.#bytes(listPointer, checkedProduct(count, 4));
        const view = new DataView(pointers.buffer, pointers.byteOffset, pointers.byteLength);
        ...
        for (let index = 0; index < count; index += 1) {
            ...
            const bindingCount = this.#exports.in_parameter_get_binding_count(pointer);
            const boundBindingCount = this.#exports.in_parameter_get_bound_binding_count(pointer);
            if (name.length === 0) {
                throw new Error(`parameter ${index} has no name`);
            }
            if (dimensions !== 1 && dimensions !== 2) {
                throw new Error(`parameter ${name} has unsupported dimensionality ${dimensions}`);
            }
            const lowerBounds = this.#readFloatVector(this.#exports.in_parameter_get_lower_bounds(pointer), dimensions);
            const upperBounds = this.#readFloatVector(this.#exports.in_parameter_get_upper_bounds(pointer), dimensions);
            for (let axis = 0; axis < dimensions; axis += 1) {
                if (!Number.isFinite(lowerBounds[axis]) ||
                    !Number.isFinite(upperBounds[axis]) ||
                    lowerBounds[axis] >= upperBounds[axis]) {
                    throw new Error(`parameter ${name} has invalid bounds on axis ${axis}`);
                }
            }
            if (!Number.isInteger(bindingCount) ||
                bindingCount < 0 ||
                bindingCount > MAX_DRAW_COMMANDS) {
                throw new Error(`parameter ${name} has an invalid binding count ${bindingCount}`);
            }
            if (!Number.isInteger(boundBindingCount) ||
                boundBindingCount < 0 ||
                boundBindingCount > bindingCount) {
                throw new Error(`parameter ${name} has an invalid resolved binding count ${boundBindingCount}`);
            }
    ...
    #readFloatVector(pointer, length) {
        const bytes = this.#bytes(pointer, length * Float32Array.BYTES_PER_ELEMENT);
        const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
        return Array.from({ length }, (_, index) => view.getFloat32(index * 4, true));
    }
    #readDrawList(puppet) {
        ...
        const commandPointer = this.#exports.in_drawlist_get_commands(drawList, outPointer);
        const commandCount = new DataView(this.#exports.memory.buffer).getUint32(outPointer, true);
        if (commandCount > MAX_DRAW_COMMANDS) {
            throw new Error(`draw command count exceeds the limit: ${commandCount}`);
        }
        const commandBytes = this.#bytes(commandPointer, checkedProduct(commandCount, DRAW_COMMAND_BYTES));
        const commandView = new DataView(commandBytes.buffer, commandBytes.byteOffset, commandBytes.byteLength);
        const commands = [];
        for (let index = 0; index < commandCount; index += 1) {
            ...
            const sources = Array.from({ length: 8 }, (_, sourceIndex) => commandView.getUint32(base + sourceIndex * 4, true));
            const variables = new Float32Array(16);
            for (let variable = 0; variable < variables.length; variable += 1) {
                variables[variable] = commandView.getFloat32(base + 64 + variable * 4, true);
                if (!Number.isFinite(variables[variable])) {
                    throw new Error(`draw command ${index} contains a non-finite variable at ${variable} ` +
        ...
        const vertexPointer = this.#exports.in_drawlist_get_vertex_data(drawList, outPointer);
        const vertexBytes = new DataView(this.#exports.memory.buffer).getUint32(outPointer, true);
        if (vertexBytes > MAX_DRAW_BUFFER_BYTES ||
        ...
        const indexPointer = this.#exports.in_drawlist_get_index_data(drawList, outPointer);
        const indexBytes = new DataView(this.#exports.memory.buffer).getUint32(outPointer, true);
        if (indexBytes > MAX_DRAW_BUFFER_BYTES ||
    ...
    #scratch(bytes) {
        ...
        const pointer = this.#scratchPointer === 0
    ...
    #float32(pointer, byteLength) {
        assertMemoryRange(this.#exports.memory, pointer, byteLength);
        return new Float32Array(this.#exports.memory.buffer, pointer, byteLength / Float32Array.BYTES_PER_ELEMENT);
    }
    #uint32(pointer, byteLength) {
        assertMemoryRange(this.#exports.memory, pointer, byteLength);
        return new Uint32Array(this.#exports.memory.buffer, pointer, byteLength / Uint32Array.BYTES_PER_ELEMENT);
    }
    #string(pointer) {
        if (pointer === 0) {
            return "";
...
function snapshotDrawList(drawList) {
    return {
...
function drawListsDiffer(before, after) {
    if (before.vertices.length !== after.vertices.length ||
    ...
    for (let index = 0; index < before.commands.length; index += 1) {
        ...
        if (left.sources.some((source, sourceIndex) => source !== right.sources[sourceIndex])) {
            return true;
        }
        for (let variable = 0; variable < left.variables.length; variable += 1) {
            if (Math.abs(left.variables[variable] - right.variables[variable]) > 1e-6) {
                return true;
            }
        }
    }
...
function assertOfficialExports(exports) {
    const required = [
        "memory",
        "in_init",
        "nu_malloc",
...
function clampParameterValue(parameter, axis, value) {
    if (!Number.isFinite(value)) {
        throw new TypeError("parameter values must be finite numbers");
    }
    return Math.min(parameter.upperBounds[axis], Math.max(parameter.lowerBounds[axis], value));
}
async function fetchBounded(url, maximumBytes, signal) {
    const request = { credentials: "same-origin" };
    if (signal) {
...
function assertMemoryRange(memory, pointer, byteLength) {
    if (!Number.isInteger(pointer) ||
        ...
        throw new Error(`official SDK returned an invalid memory range ${pointer}+${byteLength}`);
    }
}
...
function validateDrawRanges(commands, vertexCount, indexCount) {
    for (const [position, command] of commands.entries()) {
        ...
        if (command.vertexOffset > vertexCount ||
            command.indexOffset > indexCount ||
            command.elementCount > indexCount - command.indexOffset) {
            throw new Error(`draw command ${position} references data outside the draw buffers ` +
