 ...
 */
export function applyInochiBlendMode(
...
interface ProgramHandles {
  ...
  screenSpace: WebGLUniformLocation | null;
}

interface PartProgramHandles extends ProgramHandles {
  albedo: WebGLUniformLocation | null;
  mask: WebGLUniformLocation | null;
  hasMask: WebGLUniformLocation | null;
  targetSize: WebGLUniformLocation | null;
  tint: WebGLUniformLocation | null;
  screenTint: WebGLUniformLocation | null;
  opacity: WebGLUniformLocation | null;
}

interface MaskProgramHandles extends ProgramHandles {
  albedo: WebGLUniformLocation | null;
}

interface ColorBurnProgramHandles extends PartProgramHandles {
  backdrop: WebGLUniformLocation | null;
}

interface CombineProgramHandles {
  program: WebGLProgram;
  parentMask: WebGLUniformLocation | null;
  childMask: WebGLUniformLocation | null;
}

export class OfficialWebGlRenderer {
  readonly #canvas: HTMLCanvasElement;
  readonly #gl: WebGL2RenderingContext;
  readonly #meshVertexArray: WebGLVertexArrayObject;
  readonly #meshVertexBuffer: WebGLBuffer;
  readonly #meshIndexBuffer: WebGLBuffer;
  readonly #quadVertexArray: WebGLVertexArrayObject;
  readonly #quadVertexBuffer: WebGLBuffer;
  readonly #partProgram: PartProgramHandles;
  readonly #colorBurnProgram: ColorBurnProgramHandles;
  readonly #maskProgram: MaskProgramHandles;
  readonly #combineProgram: CombineProgramHandles;
  readonly #textures = new Map<number, WebGLTexture>();
  readonly #colorTargets: RenderTarget[] = [];
  readonly #maskTargets: RenderTarget[] = [];
  readonly #compositeStack: RenderTarget[] = [];
  readonly #maskStack: MaskFrame[] = [];
  readonly #whiteTexture: WebGLTexture;
  #colorTargetCursor = 0;
  #maskTargetCursor = 0;
  #lastComposite: RenderTarget | undefined;
  #logicalWidth = 1;
  #logicalHeight = 1;
  #cameraX = 0;
  #cameraY = 0;
  ...
  uploadTextures(textures: readonly OfficialTexture[]): void {
    this.#assertActive();
    const next = new Map<number, WebGLTexture>();
    try {
      for (const source of textures) {
        if (next.has(source.pointer)) {
          throw new Error(
            `official SDK returned duplicate texture pointer ${source.pointer}`,
          );
        }
  ...
  resize(width: number, height: number, devicePixelRatio: number): void {
    this.#assertActive();
    if (
      ![width, height, devicePixelRatio].every(Number.isFinite) ||
      width <= 0 ||
      height <= 0 ||
      devicePixelRatio <= 0
    ) {
  ...
  render(drawList: OfficialDrawList): void {
    this.#assertActive();
    const gl = this.#gl;
  ...
  #drawPart(command: OfficialDrawCommand, screenSpace: boolean): void {
    ...
    gl.uniform3f(
      program.tint,
      finiteOr(command.variables[0], 1),
      finiteOr(command.variables[1], 1),
      finiteOr(command.variables[2], 1),
    );
    gl.uniform3f(
      program.screenTint,
      finiteOr(command.variables[3], 0),
      finiteOr(command.variables[4], 0),
      finiteOr(command.variables[5], 0),
    );
    gl.uniform1f(program.opacity, clamp(finiteOr(command.variables[7], 1), 0, 1));
    this.#drawMesh(command);
  }

  #drawMask(command: OfficialDrawCommand): void {
    if (command.elementCount === 0) {
      return;
    }
    const gl = this.#gl;
  ...
  #blitComposite(command: OfficialDrawCommand): void {
  ...
  #drawColorBurn(
    command: OfficialDrawCommand,
    screenSpace: boolean,
    compositeBlit: boolean,
  ): void {
    const gl = this.#gl;
  ...
  #drawMesh(command: OfficialDrawCommand): void {
    const gl = this.#gl;
    gl.bindVertexArray(this.#meshVertexArray);
    gl.bindBuffer(gl.ARRAY_BUFFER, this.#meshVertexBuffer);
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.#meshIndexBuffer);
    ...
    gl.drawElements(
      ...
      gl.UNSIGNED_INT,
      command.indexOffset * Uint32Array.BYTES_PER_ELEMENT,
    );
  }
  ...
  #activeMask(): RenderTarget | undefined {
  ...
  #activeMaskAt(index: number): RenderTarget {
    const frame = this.#maskStack[index];
    if (!frame?.initialized) {
  ...
  #setCommonUniforms(program: ProgramHandles, screenSpace: boolean): void {
    const gl = this.#gl;
  ...
  #deleteTargets(targets: RenderTarget[]): void {
...
function createPartProgram(gl: WebGL2RenderingContext): PartProgramHandles {
...
function createColorBurnProgram(
  gl: WebGL2RenderingContext,
): ColorBurnProgramHandles {
...
function createMaskProgram(gl: WebGL2RenderingContext): MaskProgramHandles {
...
function createCombineProgram(gl: WebGL2RenderingContext): CombineProgramHandles {
...
function textureFormat(
  gl: WebGL2RenderingContext,
  channels: OfficialTexture["channels"],
...
function requireGlObject<T>(value: T | null, description: string): T {
  if (!value) {
